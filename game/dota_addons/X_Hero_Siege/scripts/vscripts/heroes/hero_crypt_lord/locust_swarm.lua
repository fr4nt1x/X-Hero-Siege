...
function LocustSwarmStart( event )
	...
	if ability:GetLevel() == 1 then
		unit_name = "npc_crypt_lord_locust_I"
	else
		unit_name = "npc_crypt_lord_locust_II"
	end

	-- Initialize the table to keep track of all locusts
	caster.swarm = {}
	DebugPrint("Spawning "..locusts.." locusts")
	for i=1,locusts do
	...
	unit:OnPhysicsFrame(function(unit)
		...
		if unit.state == "acquiring" then
			...
			if target_enemy then
				...
				DebugPrint("Acquiring -> Enemy Target acquired: "..unit.current_target:GetUnitName())
			...
			-- If no enemies, set the unit to collide with a random point.
				...
				DebugPrint("Acquiring -> Random Point Target acquired")
		...
		elseif unit.state == "target_acquired" then
			...
			if distance_to_caster > give_up_distance then
				unit.state = "acquiring"
				DebugPrint("Gave up on the target, acquiring a new target.")
			...
			if collision then
				...
				if unit.current_target ~= nil then
					...
					if unit.damage_done >= locust_heal_threshold then
						...
						point = source
						DebugPrint("Returning to caster after dealing ",unit.damage_done)
					else
						unit.state = "acquiring"
						DebugPrint("Attacked but still needs more damage to return: ",unit.damage_done)
					end

				-- In other case, its a point, reacquire target
				else
					unit.state = "acquiring"
					DebugPrint("Attempting to acquire a new target")
				end
			end
